...
namespace pse {
    ...
    public:
        float getRefreshDelay() const noexcept { return _refreshTimer.getDuration().count(); }

        void setRefreshDelay(float delay) noexcept { _refreshTimer.setDuration(std::chrono::duration<float>(delay)); }

        float getOriginalRefreshDelay() const noexcept { return _originalRefreshDelay.count(); }
